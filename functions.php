...
// function my_theme3_customize_register($wp_customize) {
//     // Add a section for the slider
//     $wp_customize->add_section('slider_section', array(
//         'title' => __('Slider', 'my_theme3'),
//         'priority' => 30,
//     ));
...
//             'label' => __("Slider Image $i", 'my_theme3'),
...
//             'label' => __("Slider Caption $i", 'my_theme3'),
//             'section' => 'slider_section',
...
//     }
// }
// add_action('customize_register', 'my_theme3_customize_register');





function mytheme3_customize_register( $wp_customize ) {
    // Add Slider Section
    $wp_customize->add_section('slider_section', array(
        'title'    => __('Homepage Slider', 'mytheme3'),
        'priority' => 30,
    ));

    // Add Slider Settings and Controls
    for ($i = 1; $i <= 5; $i++) {
        $wp_customize->add_setting("slider_image_$i", array(
            'default'           => '',
            'sanitize_callback' => 'esc_url_raw',
        ...
        $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, "slider_image_$i", array(
            'label'    => __("Slider Image $i", 'mytheme3'),
            'section'  => 'slider_section',
            'settings' => "slider_image_$i",
        )));
        ...
        $wp_customize->add_control("slider_caption_$i", array(
            'label'    => __("Slider Caption $i", 'mytheme3'),
            'section'  => 'slider_section',
            'type'     => 'text',
        ));
    }
}
add_action('customize_register', 'mytheme3_customize_register');
function mytheme3_enqueue_slider_scripts() {
    if (is_front_page()) {
        ...
        // Enqueue Custom Slider Initialization Script
        wp_enqueue_script('custom-slider', get_template_directory_uri() . '/js/custom-slider.js', array('slick-slider'), null, true);
    }
}
add_action('wp_enqueue_scripts', 'mytheme3_enqueue_slider_scripts');
...
function university_customize_register($wp_customize) {
    ...
    $wp_customize->add_section('our_mission_section', array(
        'title'    => __('Our Mission', 'your-theme-textdomain'),
        'priority' => 30,
    ));

    // Setting for Mission Heading
    $wp_customize->add_setting('our_mission_heading', array(
        'default'   => 'Our Mission',
        'transport' => 'refresh',
    ));

    $wp_customize->add_control('our_mission_heading', array(
        'label'   => __('Mission Heading', 'your-theme-textdomain'),
        'section' => 'our_mission_section',
        'type'    => 'text',
    ));

    // Setting for Mission Text
    $wp_customize->add_setting('our_mission_text', array(
        'default'   => 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Rerum pariatur fuga eveniet soluta aspernatur rem, nam voluptatibus voluptate voluptates sapiente, inventore. Voluptatem, maiores esse molestiae.',
        'transport' => 'refresh',
    ));

    $wp_customize->add_control('our_mission_text', array(
        'label'   => __('Mission Text', 'your-theme-textdomain'),
        'section' => 'our_mission_section',
        'type'    => 'textarea',
    ));
    ...
    // Setting for Mission Image
    $wp_customize->add_setting('our_mission_image', array(
        'default'   => '', // Default empty or set a default image URL
        'transport' => 'refresh',
    ));

    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'our_mission_image', array(
        'label'    => __('Mission Image', 'your-theme-textdomain'),
        'section'  => 'our_mission_section',
        'settings' => 'our_mission_image',
    )));
}
add_action('customize_register', 'university_customize_register');

function register_my_custom_shortcode() {
    add_shortcode('theme-my-login', 'my_custom_shortcode_function');
}
